...
#if defined(NDEBUG)
#error "SynteX cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in usyx) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight < Params().LAST_POW_BLOCK())
        ...
        nSubsidy = 150 * COIN;
    else if (nHeight > 5000 && nHeight <= 20000)
        nSubsidy = 100 * COIN;
    else if (nHeight > 20000 && nHeight <= 1050000)
        nSubsidy = 10 * COIN;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // SYX: We use certain sporks during IBD, so check to see if they are
